...
const MIGRATION_MAP = {
  ...
  'CareerSection': { targetItemTag: 'CareerItem', arrayProp: 'items', contentProp: 'description' },
  'JobSection': { targetItemTag: 'CareerItem', arrayProp: 'items', contentProp: 'description' },
};
                ...
                if (initializer && ts.isJsxExpression(initializer)) {
                    ...
                    if (expr && ts.isJsxFragment(expr)) {
                        if (expr.children.every(c => ts.isJsxText(c))) {
                            ...
                            newAttributes.push(ts.factory.createJsxAttribute(attr.name, ts.factory.createStringLiteral(text.trim())));
                            transformed = true;
                            return;
                        }
                    }
