...
export default function HRLocationsSection() {
  ...
  const fetchLocations = useCallback(async (isSilent = false) => {
    try {
      ...
      if (res.ok) {
        ...
        setError(
          "Unable to load employee locations. Please ensure you are logged in.",
        );
      }
    } catch (err) {
  ...
  const trackedCount = employees.filter(
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white rounded-2xl p-5 border border-slate-200/80 shadow-sm">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Configure per-employee location monitoring, view live GPS
            coordinates on the interactive map, and inspect historical audit
            trails.
          </p>
        </div>
