...
const CaseStudyList = ({ caseStudies }: CaseStudyListProps) => {
  ...
  const filterOptions = useMemo(() => {
    const services = new Set(caseStudies.map((cs) => cs.service));
  ...
  const filteredCaseStudies = useMemo(() => {
    ...
    return caseStudies.filter((cs) => cs.service === currentFilter);
  }, [caseStudies, currentFilter]);
        ...
        <div className="flex flex-col items-center gap-8 mb-12">
          ...
          <div className="flex flex-col items-center gap-2 sm:gap-3 mt-4">
            ...
            <h2 className="uppercase text-xs sm:text-sm font-medium text-center tracking-widest text-muted-foreground">
              {currentFilter === "All" ? "All Case Studies" : currentFilter}
            </h2>
          </div>
        </div>
