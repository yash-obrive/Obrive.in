...
export default function VacationsCalendar() {
  ...
  const fetchAllData = useCallback(async () => {
    try {
      ...
      const [empRes, dashRes] = await Promise.all([
        apiFetch("/vacations"),
        apiFetch(
          `/leaves/dashboard?date=${new Date().toISOString().split("T")[0]}`,
        ),
      ]);
  ...
  const getLeaveCount = (empId: number, type: string) => {
    ...
    return (
      emp?.leaves?.filter(
        (l) => l.leave_type === type && l.status === "approved",
      ).length || 0
    );
  };
        ...
        {["dashboard", "employees", "calendar"].map((tab) => (
          <button
            ...
            className={`px-6 py-2 rounded-xl font-semibold capitalize transition-all ${
              activeTab === tab
                ? "bg-[#074139] text-white shadow-md"
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-100 p-4">
          <div className="bg-white p-8 rounded-3xl shadow-2xl w-full max-w-md border border-gray-100">
            ...
            <p className="text-sm text-gray-500 mb-6">
              Fill in the details for your time off
            </p>
