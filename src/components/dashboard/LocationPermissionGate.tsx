...
import { ShieldAlert, MapPin, RefreshCw, AlertTriangle } from 'lucide-react';
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-md p-4">
        <div className="relative max-w-lg w-full bg-white rounded-3xl p-8 shadow-2xl border border-rose-100 text-center animate-in fade-in zoom-in duration-300">
          ...
          <p className="mt-3 text-sm text-slate-600 leading-relaxed">
            Your organization requires location verification during active work hours. 
            Location tracking is currently <strong className="text-slate-900 font-semibold">enabled for your account</strong> by HR, but your browser is blocking access.
          </p>
          ...
          <div className="mt-6 rounded-2xl bg-slate-50 p-5 text-left border border-slate-200/70">
            ...
            <ol className="space-y-2 text-xs text-slate-700 font-medium list-decimal list-inside">
              <li>Click the <strong className="text-slate-900">Tune / Lock icon</strong> on the left side of your browser address bar.</li>
              <li>Toggle <strong className="text-slate-900">Location</strong> to <strong className="text-emerald-600 font-bold">Allow</strong>.</li>
              <li>Click the button below to verify and unlock your workspace.</li>
            </ol>
          </div>
          ...
          <div className="mt-8 flex flex-col sm:flex-row gap-3">
            ...
            >
              <RefreshCw className={`h-4 w-4 ${requestingPermission ? 'animate-spin' : ''}`} />
              {requestingPermission ? 'Verifying Access...' : 'Verify & Unlock Workspace'}
            </button>
          </div>

          <p className="mt-4 text-[11px] text-slate-400">
            Once location permission is granted, your dashboard will immediately unlock.
          </p>
        </div>
      </div>
    );
