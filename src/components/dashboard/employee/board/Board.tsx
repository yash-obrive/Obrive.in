...
export default function Board({
  ...
  const taskColumnId =
    getTaskColumnId ??
    ((task: BoardTask) =>
      mode === "tasks" ? task.status || "" : task.note_date || "");
    ...
    try {
      if (onTaskDrop) {
        ...
        if (updatedTask) {
          setTasks((prev: any[]) =>
            prev.map((t) =>
              t.id === draggedTask.id ? { ...t, ...updatedTask } : t,
            ),
          );
        }
      } else if (mode === "notes") {
      ...
      {mode === "tasks" ? (
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          {columns.map((col) => (
            ...
            >
              <h2 className="text-sm font-semibold text-[#526274]">
                {col.label}
              </h2>
            </div>
          ))}
          ...
          columns.map((col) => {
            const columnEl = (
              <Column
                ...
                userId={userId}
                handleAddCardOpen={
                  shouldShowAddTask ? handleAddCardOpen : undefined
                }
                mode={mode}
