...
if (!content.includes('import { getAllBlogs }')) {
  content = content.replace('import type { StaticImageData } from "next/image";', 'import type { StaticImageData } from "next/image";\nimport { getAllBlogs } from "@/lib/blogs";');
}

// 2. Change `export const BlogCardContent = [` to `const hardcodedContent = [`
content = content.replace('export const BlogCardContent = [', 'const hardcodedContent = [');
...
// 4. Update the type
content = content.replace('src: StaticImageData;', 'src: StaticImageData | string;');
