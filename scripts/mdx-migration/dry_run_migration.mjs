...
const MIGRATION_MAP = {
  'WhyItWorkedSection': { targetItemTag: 'WhyItWorkedItem', arrayProp: 'items', contentProp: 'description' },
  'CompanyInfoSection': { targetItemTag: 'CompanyInfoItem', arrayProp: 'items', contentProp: 'description' }
};
...
function transformJsxSnippet(jsxString, config) {
    ...
    if (ts.isJsxElement(node) || ts.isJsxSelfClosingElement(node)) {
      ...
      if (cfg) {
        ...
        attributes.properties.forEach(attr => {
          if (ts.isJsxAttribute(attr) && attr.name.getText() === cfg.arrayProp) {
            transformed = true;
  ...
  if (modified) {
    ...
    return printer.printNode(ts.EmitHint.Unspecified, result.statements[0], sourceFile);
  }
  return null;
}
...
const ast = fromMarkdown(code, { extensions: [mdxjs()], mdastExtensions: [mdxFromMarkdown()] });
...
const traverse = (node) => {
  if (node.type === 'mdxJsxFlowElement' || node.type === 'mdxJsxTextElement') {
    if (MIGRATION_MAP[node.name]) {
      ...
      if (transformedSnippet) ms.overwrite(node.position.start.offset, node.position.end.offset, transformedSnippet);
    }
  }
