...
const MIGRATION_MAP = {
  'ResourceStrategicApproachSection': { targetItemTag: 'StrategicStepItem', arrayProp: 'steps', contentProp: 'description', isArray: true },
  'StrategicApproachSection': { targetItemTag: 'StrategicStepItem', arrayProp: 'steps', contentProp: 'description', isArray: true },
    ...
    if (ts.isJsxElement(node) || ts.isJsxSelfClosingElement(node)) {
      ...
      if (cfg) {
        ...
        attributes.properties.forEach(attr => {
            ...
            if (ts.isArrayLiteralExpression(expr)) {
              expr.elements.forEach((element, index) => {
                if (ts.isObjectLiteralExpression(element)) {
                  let itemProps = [];
                  let itemChildren = [];
                  itemProps.push(ts.factory.createJsxAttribute(ts.factory.createIdentifier('stepNumber'), ts.factory.createStringLiteral(String(index + 1))));
          ...
          } else if (cfg.isSingle && ts.isJsxAttribute(attr) && attr.name.getText() === cfg.singleProp) {
            transformed = true;
            const initializer = attr.initializer;
            let expr = initializer;
            if (ts.isJsxExpression(initializer)) expr = initializer.expression;
