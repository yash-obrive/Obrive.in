      ...
      if (
        ...
        if (metricsAttr || dataAttr) {
          ...
          if (ts.isJsxElement(node)) {
            ...
            // remove trailing '/>'
            originalOpeningText = originalOpeningText
              .substring(0, originalOpeningText.length - 2)
              .trim();
            originalOpeningText += ">";
          }
          ...
          if (metricsAttr) {
            originalOpeningText = originalOpeningText.replace(
              metricsAttr.getText(sourceFile),
              "",
            );
          }
          if (dataAttr) {
            originalOpeningText = originalOpeningText.replace(
              dataAttr.getText(sourceFile),
              "",
            );
          }
