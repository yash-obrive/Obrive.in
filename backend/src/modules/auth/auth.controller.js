...
exports.logout = async (req, res, next) => {
  try {
    ...
    try {
      const activeSession = await workSessionService.getTodaySession(
        req.user.id,
      );

      if (activeSession && activeSession.status === "active") {
        await workSessionService.endSession(req.user.id, activeSession.id);

        console.log(`⏹️ Work session stopped for user ${req.user.id}`);
      }
    } catch (err) {
      console.error("❌ Failed to stop session during logout:", err);
    }
