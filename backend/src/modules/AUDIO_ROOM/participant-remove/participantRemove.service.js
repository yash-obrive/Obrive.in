...
const removeParticipantService = async (payload) => {
  ...
  if (normalizeRole(participant.roomRole) === "admin") {
    throw new Error("Cannot remove admin from room");
  }
  ...
  try {
    await roomService.removeParticipant(roomId.toString(), userId.toString());
  } catch (lkError) {
    console.error(
