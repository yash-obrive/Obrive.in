...
const createRoomConfigService = async (payload, userId) => {
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    const createdRoom = await tx.room_configs.create({
      data: {
        roomName: roomConfig.roomName,

        roomDescription: roomConfig.roomDescription,

        roomType: roomConfig.roomType,

        roomStatus: roomConfig.roomStatus,

        startTime: roomConfig.startTime ? new Date(roomConfig.startTime) : null,

        endTime: roomConfig.endTime ? new Date(roomConfig.endTime) : null,

        participantLimit: roomConfig.participantLimit,

        visibility: roomConfig.visibility,

        allowGuestUsers: roomConfig.allowGuestUsers,

        redirectAfterRoomEnd: roomConfig.redirectAfterRoomEnd,
    ...
    const creatorHasExplicitAssignment = normalizedRoleAssignments.some(
      (role) =>
        role.assignmentType === "specific-user" &&
        Number(role.userId) === Number(userId),
    );

    const creatorExplicitAssignment = normalizedRoleAssignments.find(
      (role) =>
        role.assignmentType === "specific-user" &&
        Number(role.userId) === Number(userId),
    );
    ...
    if (joinPermissions?.length) {
      await tx.room_join_permissions.createMany({
        data: joinPermissions.map((permission) => ({
          roomConfigId: createdRoom.id,

          permissionType: permission.permissionType,

          crmRole: permission.crmRole || null,
        })),
      });
    }
    ...
    console.table(roleAssignments || []);

    console.table(joinPermissions || []);
