...
const joinRoomService = async (payload) => {
  const { roomId, userId, passkey } = payload;
  ...
  console.log("JOIN USER:", user.id, user.role);
  ...
  async function addParticipant(roomRole) {
    await prisma.$transaction(async (tx) => {
      const activeParticipants = await tx.room_participants.findMany({
        where: {
      ...
      const [primaryParticipant, ...duplicateParticipants] = activeParticipants;

      if (duplicateParticipants.length) {
        await tx.room_participants.updateMany({
          where: {
            id: {
              in: duplicateParticipants.map((participant) => participant.id),
            },
          },
  ...
  if (roomRole) {
    ...
    const livekitToken = await createLiveKitToken({
      ...
      participantName: user.name || user.username || `user-${user.id}`,
