...
exports.startSession = async (req, res) => {
  try {
    ...
    return successResponse(res, session, "Session initialized successfully");
  } catch (err) {
    console.error("❌ Start session controller error:", err);

    return errorResponse(res, err.message || "Failed to start session", 500);
  }
};
...
exports.heartbeat = async (req, res) => {
  try {
    ...
    return successResponse(res, session, "Heartbeat recorded");
  } catch (err) {
    console.error("❌ Heartbeat controller error:", err);

    return errorResponse(res, err.message || "Heartbeat failed", 500);
  }
};
...
exports.endSession = async (req, res) => {
  try {
    ...
    return successResponse(res, session, "Session ended successfully");
  } catch (err) {
    console.error("❌ End session controller error:", err);

    return errorResponse(res, err.message || "Failed to end session", 500);
  }
};
...
exports.getTodaySession = async (req, res) => {
  try {
    ...
    return successResponse(res, session, "Today session fetched successfully");
  } catch (err) {
    console.error("❌ Get session controller error:", err);

    return errorResponse(res, err.message || "Failed to fetch session", 500);
  }
};
