...
exports.addAvailabilitySlot = async (req, res, next) => {
  try {
    successResponse(
      res,
      await service.addAvailabilitySlot(req.user.id, req.body),
      "Slot added",
      201,
    );
  } catch (err) {
...
exports.updateAvailabilitySlot = async (req, res, next) => {
  try {
    successResponse(
      res,
      await service.updateAvailabilitySlot(
        req.params.slotId,
        req.user.id,
        req.body,
      ),
...
exports.deleteAvailabilitySlot = async (req, res, next) => {
  try {
    successResponse(
      res,
      await service.deleteAvailabilitySlot(req.params.slotId, req.user.id),
    );
  } catch (err) {
    next(err);
  }
...
exports.getEmployeeAvailability = async (req, res, next) => {
  try {
    successResponse(
      res,
      await service.getAvailability(req.params.employeeId, req.query.date),
    );
  } catch (err) {
    next(err);
  }
...
exports.getMyProjects = async (req, res, next) => {
  try {
    successResponse(res, await service.getMyProjects(req.user.id));
  } catch (err) {
    next(err);
  }
};
...
exports.getMyLogs = async (req, res, next) => {
  try {
    successResponse(res, await service.getMyLogs(req.user.id));
  } catch (err) {
    next(err);
  }
};
...
exports.recordLocation = async (req, res, next) => {
  try {
    ...
    successResponse(
      res,
      result,
      result.recorded ? "Location recorded" : result.message,
    );
  } catch (err) {
    next(err);
  }
